  ...
  return (
    <>
      <section className={styles.filters} aria-label="Project filters">
        <div className="field-container">
          ...
          <div className={styles.filterRow}><p>Trade</p><div role="group" aria-label="Filter work by trade" className={styles.chips}>
            <button type="button" onClick={() => setServiceFilter('all')} className={chip(serviceFilter === 'all')} aria-pressed={serviceFilter === 'all'}>All trades</button>
            {services.map(s => <button type="button" key={s.slug} onClick={() => setServiceFilter(s.slug)} className={chip(serviceFilter === s.slug)} aria-pressed={serviceFilter === s.slug}>{s.name}</button>)}
          </div></div>
      ...
      <section className={styles.gallery} aria-label="Project gallery" aria-busy={pending}>
        <div className="field-container">
          {filtered.length === 0 ? <div className={styles.empty}><p className="spec">Try a wider view</p><h2>No projects match<br />the current filters.</h2><p>Reset the filters to see every photographed project, or contact us about your own.</p><button type="button" onClick={clearFilters} className="cta cta--md">Reset filters</button></div> :
          <div className={styles.grid}>{filtered.map((photo, idx) => <button type="button" key={photo.id} onClick={() => setViewer({ index: idx, photos: filtered.map(p => ({ src: p.src, alt: p.alt })) })} className={styles.project} aria-label={'View: ' + photo.alt}>
            <span className={styles.image}><Image src={photo.src} alt={photo.alt} fill sizes="(max-width: 767px) 50vw, (max-width: 1023px) 45vw, 30vw" className="object-cover" /><span className={styles.open}><ArrowUpRight size={22} aria-hidden="true" /></span></span>
