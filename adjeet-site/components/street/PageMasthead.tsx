...
export function PageMasthead({ meta = [], title, lead, children, compact }: PageMastheadProps) {
  return (
    <section className="page-masthead relative overflow-hidden border-b border-rule bg-paper">
      ...
      {meta.length > 0 && (
        <div className="relative mx-auto w-full max-w-content px-5 pt-6 md:px-8" data-site-reveal="meta">
          <div className="spec flex flex-wrap items-baseline gap-x-6 gap-y-1 border-b border-rule pb-3 text-ink-muted">
...
>>>>>>> origin/main
        ...
        {lead && (
          <p className="mt-6 max-w-[52ch] text-base leading-relaxed text-ink-muted md:text-lg" data-site-reveal="lead">
            {lead}
          </p>
        )}
        {children && <div className="mt-8" data-site-reveal="actions">{children}</div>}
      </div>
    </section>
  )
}
